...
function InstallSketchybar {
  dialog --erase-on-exit --yesno "Install sketchybar?" 5 40
  response=$?
  if [[ "$response" == "0" ]]; then
    ShowInstallingMessage "sketchybar"
...
function InstallCasks {
  ...
  dialog --erase-on-exit --checklist "Select apps for install.\nUse Spacebar for selection." 20 80 30 \
    ...
    "telegram" "Telegram app" 'off' \
    "whatsapp" "WhatsApp app" 'off' \
    "slack" "Slack app" 'off' \
    ...
    "1clipboard" "clipboard manager" 'on' \
    2>tempfile
  array=($(cat tempfile))
...
function InstallTools {
  dialog --erase-on-exit --checklist "Select tools for install.\nUse Spacebar for selection." 20 80 30 \
    ...
    "posting" "Terminal HTTP client" 'off' \
    2>tempfile
  array=($(cat tempfile))
  for ((i = 0; i < ${#array[@]}; i++)); do
    ShowInstallingMessage ${array[i]}
